...
class Box(object):
    ...
    def double(self):
        ...
        to_return = self.append_string(self.contents)
        return to_return
    ...
    def shrink(self, new_volume):
        ...
        if len(content) > self.volume:
            clipped = ''
            box = ''
            for k in range(self.volume, len(content)):
                clipped += content[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        clipped = len(self.double()) + len(self.shrink(new_volume))
        return clipped
